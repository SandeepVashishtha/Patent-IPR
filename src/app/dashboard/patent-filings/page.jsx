...
function PaymentBadge({ estimation }) {
  if (estimation === null || estimation === undefined) return <span className="text-gray-400 text-xs">—</span>;
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">
      <span className="material-symbols-outlined text-[12px]">payments</span>
      ₹{Number(estimation).toLocaleString()}
    </span>
  );
}

function AgentBadge({ agentName }) {
  if (!agentName) return <span className="text-xs text-gray-400">Unassigned</span>;
  return (
...
function FilingDetailDrawer({ filing, onClose, onDelete }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getClientPatentFilingPayment(filing.id),
      getClientPatentFilingAgent(filing.id),
    ]).then(([payRes, agRes]) => {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    setDeleteErr("");
    const result = await deleteClientPatentFiling(filing.id);
    if (result.ok) {
      onDelete(filing.id);
      onClose();
    } else {
      ...
      <div className="relative ml-auto w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <h3 className="text-lg font-bold text-[#0d1b2a] mt-2">{filing.title || "Untitled Filing"}</h3>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-gray-400 uppercase tracking-wider text-[10px]">Type</p>
              <p className="font-semibold text-[#0d1b2a] mt-1">Patent Filing</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-gray-400 uppercase tracking-wider text-[10px]">Category</p>
              <p className="font-semibold text-[#0d1b2a] mt-1">{filing.fieldOfInvention || "—"}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
          ...
          {filing.status === "DRAFT" && (
            <div className="border border-red-100 rounded-xl p-4 space-y-3">
              ...
              <p className="text-xs text-gray-500">
                This filing is still in DRAFT status and can be permanently deleted.
              </p>
              <button
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function PatentFilingsPage() {
  ...
  const load = async (p = 0, status = activeStatus, q = search) => {
    setLoading(true);
    setError("");
    const result = await getClientPatentFilings({
      page: p,
      size: PAGE_SIZE,
    ...
    if (!result.ok) {
      setError(result.data?.message || "Unable to load patent filings.");
      setFilings([]);
    } else {
      ...
      if (needsAgent.length > 0) {
        const agentResults = await Promise.all(
          needsAgent.map((f) => getClientPatentFilingAgent(f.id).then((r) => ({ id: f.id, r })))
        );
        setFilings((prev) =>
          prev.map((f) => {
  ...
  useEffect(() => { load(page, activeStatus, search); }, [page, activeStatus]);

  const handleSearch = (e) => {
    if (e.key === "Enter") { setPage(0); load(0, activeStatus, search); }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#0d1b2a]">Patent Filings</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            View, track payment estimations, and manage your patent filings.
          </p>
        </div>
        <Link
          href="/dashboard/cases/new/patent"
          className="flex items-center gap-2 bg-[#1a3d54] text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-[#153144] transition-colors shrink-0"
        >
              ...
              {filtered.map((f, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    ...
                    {f.fieldOfInvention && (
                      <p className="text-xs text-gray-400 mt-0.5">{f.fieldOfInvention}</p>
                    )}
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4">
                    <PaymentBadge estimation={f.estimation} />
                  </td>
                  <td className="px-4 py-4">
                    <AgentBadge agentName={f.assignedAgentName} />
                  </td>
